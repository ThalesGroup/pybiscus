...
def generate_model_page(model: BaseModel, templatePath: str, templateName: str, buttons_type: str, on_document_load_js: str = "" ) -> str:
    ...
    try:
        ...
        with importlib.resources.files("pybiscus.session.agent.front_end").joinpath("pybiscus.css").open('r') as file:
            css = file.read()
        with importlib.resources.files("pybiscus.session.agent.front_end").joinpath(f"{buttons_type}.html").open('r') as file:
